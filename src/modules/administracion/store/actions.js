  ...
  async obtenerUsuarios() {
    ...
    try {
      ...
      if (Array.isArray(response.data)) {
        ...
        practitioners = response.data.entry.map(entry => entry.resource).filter(Boolean);
      ...
      this.usuarios = practitioners.map(practitioner => {
        const usuario = {
          ...
          activo: practitioner.active !== undefined ? practitioner.active : true,
          idTipoUsuario: null,
        };
        ...
        if (practitioner.extension && Array.isArray(practitioner.extension)) {
          const tipoUsuarioExt = practitioner.extension.find(
            ext => ext.url === 'http://mi-servidor/fhir/StructureDefinition/id-tipo-usuario'
          );
          if (tipoUsuarioExt) {
            // Puede venir como valueString o value
            usuario.idTipoUsuario = tipoUsuarioExt.valueString || 
      ...
      this.error = error.response?.data?.message || error.message || 'Error al obtener los usuarios';
      throw error;
    } finally {
      this.loading = false;
    }
